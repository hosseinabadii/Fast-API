...
async def api_create_section(section: SectionCreate, session: SessionDep):
    return await create_section(session=session, section=section)


@router.get("/", response_model=list[Section])
async def api_get_sections(session: SessionDep):
    return await get_sections(session=session)


@router.get("/{section_id}", response_model=Section)
async def api_get_section(section_id: int, session: SessionDep):
    return await get_section(session=session, section_id=section_id)
...
async def api_update_section(
    ...
    return await update_section(session=session, section_id=section_id, section=section)


@router.delete("/{section_id}", status_code=204)
async def api_delete_section(section_id: int, session: SessionDep):
    return await delete_section(session=session, section_id=section_id)
